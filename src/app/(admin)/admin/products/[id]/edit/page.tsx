...
export default function EditProductPage() {
	...
	const [form, setForm] = useState({ slug: "", name: "", description: "", priceCents: 0, compareAtPriceCents: "", category: "", brand: "", condition: "new", sizes: "", colors: "", inStock: true });
	...
	useEffect(() => {
		...
		fetch(`/api/products/${params.id}`)
			...
			.then((p) => {
				...
				setImages(Array.isArray(p.images) ? p.images.map((img: any) => ({ url: img.url, width: img.width, height: img.height, alt: img.alt })) : []);
			})
	...
	async function onSubmit(e: React.FormEvent) {
		...
		try {
			...
			const res = await fetch(`/api/products/${params.id}`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify(payload) });
			if (res.ok) {
				setStatus("Saved");
				router.refresh();
			} else {
		...
		<main className="min-h-screen px-4 md:px-10 py-10">
			...
			<form onSubmit={onSubmit} className="grid md:grid-cols-2 gap-6 max-w-4xl">
				<label>
					...
					<input value={form.slug} onChange={(e) => setForm({ ...form, slug: e.target.value })} name="slug" required className={inputClass} />
				</label>
				<label>
					<span className={labelClass}>Name</span>
					<input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} name="name" required className={inputClass} />
				</label>
				<label>
				...
				<label>
					...
					<input value={form.brand} onChange={(e) => setForm({ ...form, brand: e.target.value })} name="brand" required className={inputClass} />
				</label>
				<label>
